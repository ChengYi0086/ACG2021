...
class Cheat_w_hpt_Trainer():
    ...
    def CFR(self,reach_probs,player_index):
        ...
        while self.game.if_game_ends() != True:
            ...
            info = self.join_a_string([self.game.num_of_each_rank,len(self.game.pile_on_table),self.output_num_of_cards(self.game.cards_dealt),self.game.cards_dealt[player_index],self.game.current_tag,self.game.one_deck[self.game.current_rank_index]])
...
class Cheat_w_hpt():
    ...
    def if_game_ends(self):
        ...
        if len(self.cards_dealt[self.hpt_player_index]) < len(self.cards_dealt[(self.hpt_player_index + 1)%2]):
            self.winning_tag = "special case"
            # Although this player has lost all the health points 
            # but it holds fewer cards, we will say it wins anyway
            self.winning_number[self.hpt_player_index] += 1
        else:
    ...
    def get_payoff(self,player_index):
        if self.winning_tag == "Discarded all cards":
            ...
            if player_index == self.hpt_player_index:
                ...
                return 1.0
        elif self.winning_tag == "special case":
            if player_index == self.hpt_player_index:
                return 1.0
            else:
                return -1.0
