import copy,random,sys,os,ast,time,pickle
import numpy as np
...
class Cheat_w_hpt_Player():
    ...
    def discard(self,num_of_each_rank,pile_on_table,cards_dealt,player_index,history_claim,current_rank):
        info = self.join_a_string([num_of_each_rank,len(pile_on_table),self.output_num_of_cards(cards_dealt),cards_dealt[player_index],history_claim,current_rank])
        infoset = self.get_infoset(info)
        action = infoset.getActionfromStrategy()
        return action

    def challenge(self,num_of_each_rank,pile_on_table,cards_dealt,player_index,history_claim,current_rank):
        info = self.join_a_string([num_of_each_rank,len(pile_on_table),self.output_num_of_cards(cards_dealt),cards_dealt[player_index],history_claim,current_rank])
        infoset = self.get_infoset(info)
        action = infoset.getActionfromStrategy()
        return action
...
class Cheat_w_hpt_Trainer():
    ...
    def CFR(self,reach_probs,player_index):
        while self.game.if_game_ends() != True:
            ...
            info = self.join_a_string([self.game.num_of_each_rank,len(self.game.pile_on_table),self.output_num_of_cards(self.game.cards_dealt),self.game.cards_dealt[player_index],self.game.history_claim,self.game.one_deck[self.game.current_rank_index]])
            ...
            while self.if_game_ends() != True:
                ...
                if self.check_state() == "The Game Begins" or self.check_state() == "Discard phase":
                    ...
                    if self.players_list[self.discard_player_index] == "CFR player" or self.players_list[self.discard_player_index] == "CFR player 1" or self.players_list[self.discard_player_index] == "CFR player 2":
                        discard_cards = self.CFR_player.discard(self.num_of_each_rank,self.pile_on_table,self.cards_dealt,self.discard_player_index,self.history_claim,self.one_deck[self.current_rank_index])
                    ...
                    if self.players_list[self.challenge_player_index] == "CFR player" or self.players_list[self.challenge_player_index] == "CFR player 1" or self.players_list[self.challenge_player_index] == "CFR player 2":
                        action = self.CFR_player.challenge(self.num_of_each_rank,self.pile_on_table,self.cards_dealt,self.challenge_player_index,self.history_claim,self.one_deck[self.current_rank_index])
