...
# for the game Cheat for the HPA agent #
...
class Cheat_w_hpt_Player():
    ...
    def discard(self,num_of_each_rank,pile_on_table,cards_dealt,player_index,current_tag,current_rank,hpt_left):
        info = self.join_a_string([num_of_each_rank,len(pile_on_table),self.output_num_of_cards(cards_dealt),cards_dealt[player_index],current_tag,current_rank,hpt_left])
        infoset = self.get_infoset(info)

        action = infoset.getActionfromStrategy()
        return action

    def challenge(self,num_of_each_rank,pile_on_table,cards_dealt,player_index,current_tag,current_rank,hpt_left):
        info = self.join_a_string([num_of_each_rank,len(pile_on_table),self.output_num_of_cards(cards_dealt),cards_dealt[player_index],current_tag,current_rank,hpt_left])
        infoset = self.get_infoset(info)
...
class Cheat_w_hpt_Trainer():
    ...
    def CFR(self,reach_probs,player_index):
        ...
        while self.game.if_game_ends() != True:
            self.game.check_state()
            self.hpt_left = [0] * self.game.num_of_players
            for playerindex in range(len(self.game.current_hpt)):
                self.hpt_left[playerindex] = self.game.num_of_hpt - self.game.current_hpt[playerindex]
                if self.hpt_left[playerindex] < 0:
                    print("ERROR: Hit point negative")
                    sys.exit()
            info = self.join_a_string([self.game.num_of_each_rank,len(self.game.pile_on_table),self.output_num_of_cards(self.game.cards_dealt),self.game.cards_dealt[player_index],self.game.current_tag,self.game.one_deck[self.game.current_rank_index],self.hpt_left])
            ...
            if self.game.check_state() == "The Game Begins" or self.game.check_state() == "Discard phase":
                ...
                action = infoset.getActionfromStrategy()
                index = infoset.getActions().index(action)

                self.game.check_state()
                #print("-----------------------------------")
                rank_state = "Current Rank: {current_rank}".format(current_rank = self.game.one_deck[self.game.current_rank_index])
                player_state = "Current Discard Player: {current_player}".format(current_player = self.game.players_list[player_index])
                self.discard_player_index = player_index
                #print(rank_state + "\n" + player_state)

                action_prob = strategy[index]
                new_reach_prob = copy.deepcopy(reach_probs)
                new_reach_prob[player_index] *= action_prob

                #print("Player {player_num} {player_name} discards {player_action}".format(player_num = player_index + 1, player_name = self.game.players_list[player_index],player_action = action))
                ...
                counterfactual_values[index] = -self.CFR(new_reach_prob,opponent_index)
                ...
                player_state = "Current Challenge Player: {current_player}".format(current_player = self.game.players_list[player_index])
                self.challenge_player_index = player_index
                #print("Player {player_num} {player_name}'s choice is {player_action}".format(player_num = player_index + 1, player_name = self.game.players_list[player_index],player_action = action))
                self.game.update_history(action)
                ...
                if action == True:
                    if self.game.check_result(len(self.discard_cards) * [self.game.one_deck[self.game.current_rank_index]],self.discard_cards):
                        ...
                        self.game.update_challenge_cards(self.challenge_player_index)
                    else:
                        #print("Challenge Successes! It's a lie.")
                        self.game.current_hpt[(player_index +1)%2] += 1
                        self.game.update_challenge_cards(self.discard_player_index)

                    #print("Now Hit Points are {hit_points}".format(hit_points = self.game.current_hpt))

                next_player_index = player_index


                counterfactual_values[index] = self.CFR(new_reach_prob,next_player_index)           
            ...
            for index, action in enumerate(infoset.getActions()):
                infoset.regretSum[index] += reach_probs[opponent_index] * (counterfactual_values[index] - infoset_value)
    ...
    def train(self,iterations):
        ...
        for iter in range(iterations):
            print("Here is {num}th iterations".format(num = iter))
            len_of_infoset_list = len(self.infoset_map.keys())
            self.game.begin_new_game()
            reach_probs = np.ones(2)
            util += self.CFR(reach_probs,random.randint(0,1))
            one_iteration_end_time = time.time()
            one_iteration_time = one_iteration_end_time - train_begin_time
...
class Cheat_w_hpt():
    def __init__(self,num_of_ranks,num_of_each_rank,num_of_cards_for_each_player,players_list,num_of_hpt,CFR_player_info):
            ...
            while self.if_game_ends() != True:
                if self.check_state() == "The Game Begins" or self.check_state() == "Discard phase":
                    ...
                    if self.players_list[self.discard_player_index] == "CFR player" or self.players_list[self.discard_player_index] == "CFR player 1" or self.players_list[self.discard_player_index] == "CFR player 2":
                        self.hpt_left = [0] * self.num_of_players
                        for player_index in range(len(self.current_hpt)):
                            self.hpt_left[player_index] = self.num_of_hpt - self.current_hpt[player_index]
                            if self.hpt_left[player_index] < 0:
                                print("ERROR: Hit point negative")
                                sys.exit()
                        discard_cards = self.CFR_player.discard(self.num_of_each_rank,self.pile_on_table,self.cards_dealt,self.discard_player_index,self.current_tag,self.one_deck[self.current_rank_index],self.hpt_left)
                    ...
                    if self.players_list[self.challenge_player_index] == "CFR player" or self.players_list[self.challenge_player_index] == "CFR player 1" or self.players_list[self.challenge_player_index] == "CFR player 2":
                        ...
                        for player_index in range(len(self.current_hpt)):
                            ...
                            if self.hpt_left[player_index] < 0:
                                print("ERROR: Hit point negative")
                                sys.exit()
                        action = self.CFR_player.challenge(self.num_of_each_rank,self.pile_on_table,self.cards_dealt,self.challenge_player_index,self.current_tag,self.one_deck[self.current_rank_index],self.hpt_left)
